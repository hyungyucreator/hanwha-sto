    ...
    <div style={{
      ...
      <input
        ...
        style={{
          ...
          fontSize: "13px", color: C.text, fontFamily: "inherit", height: "38px",
        }}
      />
      <div style={{
...
export default function App() {
  ...
  const [mobileScreen, setMobileScreen] = useState("input"); // input | result
  const [mobileResultTab, setMobileResultTab] = useState("verdict");
  ...
  const handleTypeChange = (id) => {
    setAssetType(id); setFieldValues({}); setResult(null); setError("");
  };
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setResult(r);
      if (isMobile) { setMobileScreen("result"); setMobileResultTab("verdict"); }
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const handleReset = () => {
    ...
    setResultAssetName("");
    if (isMobile) setMobileScreen("input");
  };
  ...
  const InputForm = () => (
    <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "20px 16px 16px" : "28px 24px 16px" }}>
      ...
      <div style={{ marginBottom: "18px" }}>
        ...
        {currentType?.fields.map(field => (
          <div key={field.key} style={{ marginBottom: "10px" }}>
            ...
            {field.unit ? (
              <InputWithUnit value={fieldValues[field.key] || ""} onChange={v => handleFieldChange(field.key, v)} placeholder={field.placeholder} unit={field.unit} />
            ) : (
              <Input value={fieldValues[field.key] || ""} onChange={v => handleFieldChange(field.key, v)} placeholder={field.placeholder} />
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: "8px" }}>
        ...
        <Input value={freeText} onChange={setFreeText} placeholder="특이사항, 발행사가 전달한 내용, 담당자 메모 등" multiline />
      </div>
    ...
    <div style={{ animation: "fadeUp 0.3s ease" }}>
      {!isMobile && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
          <div>
            <div style={{ fontSize: "11px", color: C.textDim, marginBottom: "3px" }}>
          ...
          <button onClick={handleReset} style={{
            ...
            borderRadius: "6px", fontSize: "12px", fontWeight: "600", color: C.textMid,
      ...
      <div style={{ marginTop: "14px", fontSize: "11px", color: C.textDim, lineHeight: "1.6" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Noto Sans KR','Apple SD Gothic Neo',sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{
        ...
        padding: "0 20px", height: headerHeight,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <div style={{ fontSize: "10px", color: C.textDim, letterSpacing: "0.12em", textTransform: "uppercase" }}>
              Hanwha Investment Securities
            </div>
            <div style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: "700", color: C.text, letterSpacing: "-0.02em", lineHeight: 1.2 }}>
              STO 자산 스크리닝 툴
            </div>
          </div>
        </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
                  {[1,2,3,4].map(i => (
                    <div key={i} style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: "10px", padding: "18px" }}>
                      <Skeleton h={10} w="50%" mb={12} /><Skeleton h={12} mb={6} /><Skeleton h={12} w="75%" mb={0} />
                    </div>
                  ))}
                </div>
              </div>
            )}
            {result && !loading && <ResultContent />}
      ...
      {/* ── 모바일 레이아웃 ── */}
      {isMobile && (
        <div style={{ height: `calc(100vh - ${headerHeight})`, display: "flex", flexDirection: "column" }}>
          ...
          {mobileScreen === "result" && (
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
              ...
              <div style={{ padding: "12px 16px", background: C.white, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={handleReset} style={{ padding: "6px 12px", background: C.white, border: `1px solid ${C.border}`, borderRadius: "6px", fontSize: "11px", fontWeight: "600", color: C.textMid, cursor: "pointer" }}>
                  ↺ 다시
                </button>
              </div>
